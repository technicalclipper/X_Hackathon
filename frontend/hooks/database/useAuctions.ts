...
export const useAuctions = () => {
  ...
  const [filterType, setFilterType] = useState<'all' | 'active' | 'ended'>('all');
  ...
  const fetchAuctions = async () => {
    try {
      ...
      const { data: nftMintsData, error: nftMintsError } = await supabase
        .from('nft_mints')
        .select(`
          id,
          submission_id,
      ...
      const joinedData = auctionsData?.map(auction => {
        const nftMint = nftMintsData?.find(nft => 
          nft.minted_token_id === auction.token_id.toString()
        );
        
        if (!nftMint) {
          console.warn(`No NFT mint found for auction token_id: ${auction.token_id}`);
          return null;
        }
      ...
      const transformedData: Auction[] = joinedData?.map((auction: any) => ({
        ...
        contract_submission_id: auction.nft_mints.submissions.contract_submission_id,
        ...
        submission_deadline: auction.nft_mints.submissions.pools.submission_deadline,
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(auction => 
        ...
        auction.seller_address.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
